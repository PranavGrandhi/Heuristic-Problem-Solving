...
def my_solution(pers, hosps, result_file):
    ...
    with open(result_file, "w") as output_file:
        ...
        # **Collect all ambulances into a list for redistribution**
        all_ambulances = []
        for h in hosps:
            for avail_time in h.amb_time:
                all_ambulances.append({'current_hospital': h, 'avail_time': avail_time})

        # Calculate total ambulances and desired number per hospital
        total_ambulances = len(all_ambulances)
        num_hospitals = len(hosps)
        equal_num_amb = total_ambulances // num_hospitals
        remainder = total_ambulances % num_hospitals

        # Assign desired number of ambulances to each hospital
        desired_ambulance_counts = []
        for i in range(num_hospitals):
            desired_count = equal_num_amb
            if remainder > 0:
                desired_count += 1
                remainder -= 1
            desired_ambulance_counts.append(desired_count)

        # Assign ambulances to hospitals to ensure equal distribution
        assigned_ambulances = {h.hid: [] for h in hosps}
        ambulance_index = 0

        for i, desired_count in enumerate(desired_ambulance_counts):
            h = hosps[i]
            for _ in range(desired_count):
                if ambulance_index < len(all_ambulances):
                    assigned_ambulances[h.hid].append(all_ambulances[ambulance_index])
                    ambulance_index += 1

        # Clear ambulances from all hospitals
        for h in hosps:
            h.amb_time = []
            h.num_amb = 0

        # **Plan movements of ambulances to their assigned hospitals, picking up people along the way**
        unrescued_persons = pers.copy()  # List of unrescued persons

        for h in hosps:
            assigned_ambs = assigned_ambulances[h.hid]
            for amb in assigned_ambs:
                current_hospital = amb['current_hospital']
                start_time = amb['avail_time']
                if current_hospital.hid != h.hid:
                    # Ambulance needs to move from current_hospital to h
                    # Plan a route picking up up to 4 people along the way
                    capacity = 4
                    current_time = start_time
                    current_location = (current_hospital.x, current_hospital.y)
                    route = [f"{current_time} H{current_hospital.hid}"]
                    picked_persons = []
                    ...
                    while capacity > 0:
                        ...
                        for p in unrescued_persons:
                            ...
                            # Travel time from person's location to destination hospital
                            travel_time_to_dest_hosp = abs(p.x - h.x) + abs(p.y - h.y)

                            # Estimate arrival time at destination hospital after picking up this person
                            arrival_time_at_dest_hosp = time_after_loading_p + travel_time_to_dest_hosp

                            # Add unloading time at hospital (1 minute)
                            total_time_if_only_p = arrival_time_at_dest_hosp + 1

                            # Check if this person can be delivered to the hospital before their survival time
                            if total_time_if_only_p <= p.st:
                                # Calculate priority score (e.g., based on distance and survival time)
                                # For simplicity, we use the travel time to person
                                priority_score = travel_time_to_p
                        ...
                        for candidate in candidates:
                            ...
                            # Travel time to destination hospital
                            travel_time_to_dest_hosp = abs(new_current_location[0] - h.x) + abs(new_current_location[1] - h.y)
                            # Total time to reach destination hospital after picking up this person
                            total_time_to_hospital = time_after_loading_p + travel_time_to_dest_hosp
                            # Add unloading time
                    ...
                    # After picking up persons, travel to destination hospital
                    travel_time_to_dest_hosp = abs(current_location[0] - h.x) + abs(current_location[1] - h.y)
                    current_time += travel_time_to_dest_hosp
                    current_location = (h.x, h.y)
                    # Unload time
                    current_time += 1
                    route.append(f"H{h.hid}")

                    # Write the route to the output file
                    output_file.write(" ".join(route) + "\n")

                    # Update ambulance availability time at the destination hospital
                    h.amb_time.append(current_time)
                    h.num_amb += 1

                    # Decrease the ambulance count at the current hospital (though we cleared them earlier)
                    # This is just for completeness
                    current_hospital.num_amb -= 1

                else:
                    # Ambulance is already at the assigned hospital
                    h.amb_time.append(start_time)
                    h.num_amb += 1

        # **From the second iteration onwards, proceed with the original approach**
